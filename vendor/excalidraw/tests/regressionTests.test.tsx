...
describe("regression tests", () => {
  ...
  it("scribble delete removes standalone text when the gesture is confident", () => {
    ...
    UI.clickTool("freedraw");
    mouse.downAt(118, 116);
    mouse.moveTo(252, 116);
    mouse.moveTo(132, 130);
    mouse.moveTo(248, 142);
    mouse.moveTo(134, 156);
    mouse.moveTo(246, 168);
    mouse.moveTo(138, 178);
    mouse.moveTo(242, 186);
    mouse.upAt(142, 192);

    const nonDeleted = h.elements.filter((element) => !element.isDeleted);
    expect(nonDeleted.some((element) => element.id === text.id)).toBe(false);
    expect(nonDeleted.some((element) => element.type === "freedraw")).toBe(
      false,
    );
  });

  it("keeps normal freedraw behavior when stroke is not scribble-like", () => {
    const text = API.createElement({
      type: "text",
      x: 100,
      y: 100,
    ...
    const nonDeleted = h.elements.filter((element) => !element.isDeleted);
    expect(nonDeleted.some((element) => element.id === text.id)).toBe(true);
    expect(
      nonDeleted.filter((element) => element.type === "freedraw").length,
    ).toBe(1);
  });

  it("keeps normal freedraw behavior when no text target qualifies", () => {
    const text = API.createElement({
      type: "text",
      x: 100,
      y: 100,
    ...
    UI.clickTool("freedraw");
    mouse.downAt(418, 116);
    mouse.moveTo(552, 116);
    mouse.moveTo(432, 130);
    mouse.moveTo(548, 142);
    mouse.moveTo(434, 156);
    mouse.moveTo(546, 168);
    mouse.moveTo(438, 178);
    mouse.moveTo(542, 186);
    mouse.upAt(442, 192);
