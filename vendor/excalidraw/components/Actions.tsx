...
const canChangeStrokeWidth = (
  appState: UIAppState,
  targetElements: ExcalidrawElement[],
) => {
  const toolHasStrokeWidth =
    appState.activeTool.type === "highlighter" ||
...
export const SelectedShapeActions = ({
  ...
  const showBackgroundColorAction =
    isDrawToolActive || canChangeBackgroundColor(appState, targetElements);
  const showStrokeWidthAction =
    isDrawToolActive || canChangeStrokeWidth(appState, targetElements);

  const showFillIcons =
    (showBackgroundColorAction &&
  ...
  return (
    <div className="selected-shape-actions">
      ...
      {showStrokeWidthAction && renderAction("changeStrokeWidth")}
  ...
  const ERASER_TOOLS = [
    {
      type: "eraserStroke",
      icon: EraserIcon,
      title: capitalizeString(t("eraserMode.stroke")),
    },
    {
      type: "eraserPoint",
      icon: PointEraserIcon,
      title: capitalizeString(t("eraserMode.point")),
    },
  ] as const;

  const eraserOption =
    app.state.eraserMode === "point" ? "eraserPoint" : "eraserStroke";
    ...
    <>
      {getToolbarTools(app).map(
        ({ value, icon, key, numericKey, fillable, toolbar }) => {
          ...
          if (value === "eraser") {
            const applyEraserMode = (option: string) => {
              const nextMode = option === "eraserPoint" ? "point" : "stroke";
              setAppState({ eraserMode: nextMode });
            ...
            return (
              <ToolPopover
                key="eraser-popover"
                app={app}
                options={ERASER_TOOLS}
                activeTool={activeTool}
                activeOptionType={eraserOption}
                triggerActive={activeTool.type === "eraser"}
                defaultOption={eraserOption}
                namePrefix="eraserMode"
                title={`${capitalizeString(t("toolBar.eraser"))} — ${capitalizeString(
                  t("eraserMode.title"),
                )}`}
                data-testid="toolbar-eraser"
                onToolChange={applyEraserMode}
                onOptionSelect={applyEraserMode}
                displayedOption={
                  ERASER_TOOLS.find((tool) => tool.type === eraserOption) ||
                  ERASER_TOOLS[0]
                }
                fillable={false}
              />
      ...
      <DropdownMenu open={isExtraToolsMenuOpen}>
        ...
        >
          ...
          <DropdownMenu.Item
            onSelect={() => app.setOpenDialog({ name: "ttd", tab: "plantuml" })}
            icon={mermaidLogoIcon}
            data-testid="toolbar-plantuml"
          >
            {t("toolBar.plantumlToExcalidraw")}
          </DropdownMenu.Item>
