...
export const MobileToolBar = ({
  ...
  const [lastActiveLinearElement, setLastActiveLinearElement] = useState<
    "arrow" | "line"
  >("arrow");
  const [lastActiveDrawTool, setLastActiveDrawTool] = useState<
  ...
  useEffect(() => {
    if (activeTool.type === "freedraw" || activeTool.type === "highlighter") {
      setLastActiveDrawTool(activeTool.type);
    }
  }, [activeTool.type]);
  ...
  return (
    ...
    >
      ...
      <ToolPopover
        ...
        options={DRAW_TOOLS}
        activeTool={activeTool}
        defaultOption={lastActiveDrawTool}
        namePrefix="drawToolType"
        title={capitalizeString(
          t(
            lastActiveDrawTool === "highlighter"
              ? "toolBar.highlighter"
              : "toolBar.freedraw",
          ),
        )}
        ...
        onToolChange={(type: string) => {
          if (type === "freedraw" || type === "highlighter") {
            setLastActiveDrawTool(type);
            app.setActiveTool({ type });
          }
        }}
        displayedOption={
          DRAW_TOOLS.find((tool) => tool.type === lastActiveDrawTool) ||
          DRAW_TOOLS[0]
        }
      />
      ...
      <ToolPopover
        ...
        displayedOption={
          ERASER_MODE_TOOLS.find(
            (tool) =>
              tool.type ===
              (app.state.eraserMode === "point"
                ? "eraserPoint"
                : "eraserStroke"),
          ) || ERASER_MODE_TOOLS[0]
        }
