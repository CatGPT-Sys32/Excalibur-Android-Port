...
const EXPERIMENTAL_SCRIBBLE_DELETE_TEXT = true;
const SCRIBBLE_DELETE_MIN_POINTS = 8;
const SCRIBBLE_DELETE_MIN_AXIS_SIZE = 28;
const SCRIBBLE_DELETE_MIN_PATH_LENGTH = 120;
const SCRIBBLE_DELETE_MIN_PATH_TO_DIAGONAL_RATIO = 2.6;
const SCRIBBLE_DELETE_MIN_DIRECTION_CHANGES = 4;
const SCRIBBLE_DELETE_MAX_DIRECTION_DOT = 0.5;
const SCRIBBLE_DELETE_MIN_INSIDE_POINTS = 6;
const SCRIBBLE_DELETE_MIN_INSIDE_RATIO = 0.55;
const SCRIBBLE_DELETE_MIN_STROKE_BOUNDS_OVERLAP = 0.5;
const HIGHLIGHTER_MAX_OPACITY = 35;
const HIGHLIGHTER_MIN_STROKE_WIDTH = 0.25;
const TRIANGLE_TOOL_SHAPE = "triangle";
...
class App extends React.Component<AppProps, AppState> {
  ...
  private eraserPathPoints: GlobalPoint[] = [];
      ...
      onSelect: (color, event) => {
        ...
        if (
          ...
          if (shouldUpdateStrokeColor) {
            const strokeColorState =
              this.state.activeTool.type === "highlighter"
                ? {
                    currentItemStrokeColor: color,
                    currentItemHighlighterStrokeColor: color,
                  }
                : {
                    currentItemStrokeColor: color,
                    currentItemFreedrawStrokeColor: color,
                  };
            this.syncActionResult({
              appState: { ...this.state, ...strokeColorState },
  ...
  private onKeyDown = withBatchedUpdates(
    (event: React.KeyboardEvent | KeyboardEvent) => {
      ...
      if (
        ...
        const shape = findShapeByKey(event.key, this, event.code);
  ...
  setActiveTool = (
    ...
    this.setState((prevState) => {
      ...
      const shouldSyncToolStrokeColor =
        nextActiveTool.type === "highlighter" ||
        nextActiveTool.type === "freedraw" ||
        prevState.activeTool.type === "highlighter";
      const strokeColorPatch = shouldSyncToolStrokeColor
        ? {
            currentItemStrokeColor:
              nextActiveTool.type === "highlighter"
                ? prevState.currentItemHighlighterStrokeColor
                : prevState.currentItemFreedrawStrokeColor,
          }
        : {};
  ...
  private handleEraser = (
    ...
    if (this.state.eraserMode === "point") {
      this.eraserTrail.addPointToPath(scenePointer.x, scenePointer.y, false);
      this.eraserPathPoints.push(
        pointFrom<GlobalPoint>(scenePointer.x, scenePointer.y),
      );
  ...
  private handleCanvasPointerDown = (
    ...
    } else if (this.state.activeTool.type === TRIANGLE_TOOL_SHAPE) {
  ...
  private handleFreeDrawElementOnPointerDown = (
    ...
    const strokeColor = isHighlighter
      ? this.state.currentItemHighlighterStrokeColor
      : this.state.currentItemFreedrawStrokeColor;
    ...
    const element = newFreeDrawElement({
      ...
      strokeWidth: isHighlighter
        ? Math.max(
            ...
            HIGHLIGHTER_MIN_STROKE_WIDTH,
          )
        : this.state.currentItemStrokeWidth,
      strokeStyle: this.state.currentItemStrokeStyle,
      roughness: this.state.currentItemRoughness,
      opacity: isHighlighter
        ? Math.min(this.state.currentItemOpacity, HIGHLIGHTER_MAX_OPACITY)
    ...
    return withBatchedUpdatesThrottled((event: PointerEvent) => {
      ...
      if (this.state.selectionElement) {
        ...
        if (newElement.type === "freedraw") {
          ...
          this.state.activeTool.type === TRIANGLE_TOOL_SHAPE &&
          isLineElement(newElement) &&
          newElement.polygon &&
          ...
          const x1 = pointerDownState.originInGrid.x;
          const y1 = pointerDownState.originInGrid.y;
          const x2 = gridX;
          const y2 = gridY;
          ...
          const apexY = y2 >= y1 ? minY : maxY;
          const baseY = y2 >= y1 ? maxY : minY;
          ...
          this.scene.mutateElement(
            newElement,
            {
              ...
              y: minY,
              points: [
                pointFrom<LocalPoint>(apexX - minX, apexY - minY),
                pointFrom<LocalPoint>(0, baseY - minY),
                pointFrom<LocalPoint>(maxX - minX, baseY - minY),
                pointFrom<LocalPoint>(apexX - minX, apexY - minY),
              ],
            },
            {
  ...
  private onPointerUpFromPointerDownHandler(
    ...
    return withBatchedUpdates((childEvent: PointerEvent) => {
      ...
      if (newElement?.type === "freedraw") {
        ...
        const scribbleDeleteTarget = this.getScribbleDeleteTextTarget(
          newElement,
        );

        if (scribbleDeleteTarget) {
          this.elementsPendingErasure = new Set([
            newElement.id,
            scribbleDeleteTarget.id,
          ]);
      ...
      const isTriangleElement =
        activeTool.type === TRIANGLE_TOOL_SHAPE &&
        isLineElement(newElement) &&
        newElement.polygon;

      if (isLinearElement(newElement) && !isTriangleElement) {
        if (
      ...
      if (isEraserActive(this.state) && pointerStart && pointerEnd) {
        ...
        if (this.state.eraserMode === "point") {
          ...
          if (!this.eraserPathPoints.length) {
            this.eraserPathPoints = [
              pointFrom<GlobalPoint>(scenePointer.x, scenePointer.y),
            ];
          } else {
            const lastPoint = this.eraserPathPoints[this.eraserPathPoints.length - 1];
            if (
              lastPoint[0] !== scenePointer.x ||
              lastPoint[1] !== scenePointer.y
            ) {
              this.eraserPathPoints.push(
                pointFrom<GlobalPoint>(scenePointer.x, scenePointer.y),
              );
          ...
          const didChange = this.eraseElementsWithPointEraser();
          this.eraserPathPoints = [];
  ...
  private eraseElementsWithPointEraser = (): boolean => {
    if (this.eraserPathPoints.length === 0) {
      return false;
    }

    const eraserPathBounds = this.getPathBounds(this.eraserPathPoints);
    if (!eraserPathBounds) {
      return false;
    }

    const eraserPathSegments = this.eraserPathPoints
      .slice(1)
      .map((point, index) =>
        lineSegment<GlobalPoint>(this.eraserPathPoints[index], point),
      );

    if (!eraserPathSegments.length) {
      return false;
    }
    ...
    this.scene.getElementsIncludingDeleted().forEach((element) => {
      ...
      const eraseTolerance = Math.max(element.strokeWidth * 0.7, 6 / zoom);
      const elementBounds = getElementBounds(element, elementsMap);

      const eraserBoundsWithTolerance = [
        eraserPathBounds[0] - eraseTolerance,
        eraserPathBounds[1] - eraseTolerance,
        eraserPathBounds[2] + eraseTolerance,
        eraserPathBounds[3] + eraseTolerance,
      ] as const;

      if (!doBoundsIntersect(eraserBoundsWithTolerance, elementBounds)) {
        nextElements.push(element);
        return;
      }

      const elementSegments = element.points.slice(1).map((point, index) =>
      ...
      const erasedSegmentIndices = new Set<number>();

      elementSegments.forEach((segment, segmentIndex) => {
        for (const eraserSegment of eraserPathSegments) {
          if (lineSegmentsDistance(segment, eraserSegment) <= eraseTolerance) {
            erasedSegmentIndices.add(segmentIndex);
            break;
          }
        }
      });

      if (!erasedSegmentIndices.size) {
        nextElements.push(element);
        return;
      }
      ...
      let currentPoints: LocalPoint[] = [element.points[0]];
      let currentPressures: number[] = element.simulatePressure
        ? []
        : [element.pressures[0] ?? 0.5];

      for (let segmentIndex = 0; segmentIndex < elementSegments.length; segmentIndex++) {
        const nextPoint = element.points[segmentIndex + 1];
        const nextPressure = element.pressures[segmentIndex + 1];

        if (erasedSegmentIndices.has(segmentIndex)) {
          ...
          currentPressures = element.simulatePressure
            ? []
            : [nextPressure ?? element.pressures[segmentIndex] ?? 0.5];
        } else {
          currentPoints = [...currentPoints, nextPoint];
      ...
      nextElements.push(
        newElementWith(element, {
          ...
          pressures: element.simulatePressure ? [] : runs[0].pressures,
        }),
      );
      ...
      runs.slice(1).forEach((run) => {
        nextElements.push(
          newFreeDrawElement({
            ...
            points: run.points,
            pressures: element.simulatePressure ? [] : run.pressures,
          }),
        );
      });
    });
  ...
  private getPathBounds = (points: readonly GlobalPoint[]) => {
    if (!points.length) {
      return null;
    }
  ...
  private getScribbleDeleteTextTarget = (
    freedrawElement: ExcalidrawFreeDrawElement,
  ): ExcalidrawTextElement | null => {
    if (!EXPERIMENTAL_SCRIBBLE_DELETE_TEXT) {
      return null;
    }

    const points = freedrawElement.points.map((point) =>
      pointFrom<GlobalPoint>(
        freedrawElement.x + point[0],
        freedrawElement.y + point[1],
      ),
    );

    if (points.length < SCRIBBLE_DELETE_MIN_POINTS) {
      return null;
    }
    ...
    for (let i = 0; i < points.length; i++) {
      ...
      if (i > 0) {
        ...
        if (i > 1) {
          const prePrevPoint = points[i - 2];
          const prevDx = prevPoint[0] - prePrevPoint[0];
          const prevDy = prevPoint[1] - prePrevPoint[1];
          const prevSegmentLength = Math.hypot(prevDx, prevDy);

          if (prevSegmentLength > 0.5 && segmentLength > 0.5) {
            const dot =
              (prevDx * dx + prevDy * dy) /
              (prevSegmentLength * segmentLength);
            if (dot <= SCRIBBLE_DELETE_MAX_DIRECTION_DOT) {
              directionChanges++;
            }
          }
        }
    ...
    const boundsDiagonal = Math.hypot(boundsWidth, boundsHeight);

    if (
      boundsWidth < SCRIBBLE_DELETE_MIN_AXIS_SIZE ||
      boundsHeight < SCRIBBLE_DELETE_MIN_AXIS_SIZE ||
      pathLength < SCRIBBLE_DELETE_MIN_PATH_LENGTH ||
      boundsDiagonal === 0 ||
      pathLength / boundsDiagonal < SCRIBBLE_DELETE_MIN_PATH_TO_DIAGONAL_RATIO ||
      directionChanges < SCRIBBLE_DELETE_MIN_DIRECTION_CHANGES
    ) {
      return null;
    }

    const strokeBounds = [minX, minY, maxX, maxY] as const;
    const strokeBoundsArea = Math.max(1, boundsWidth * boundsHeight);

    const elementsMap = this.scene.getNonDeletedElementsMap();
    const textElements = this.scene.getNonDeletedElements().filter(
      (element): element is ExcalidrawTextElement =>
        isTextElement(element) && !element.locked && !isBoundToContainer(element),
    );

    let bestTarget: ExcalidrawTextElement | null = null;
    let bestScore = 0;

    for (const textElement of textElements) {
      const textBounds = getElementBounds(textElement, elementsMap);

      if (!doBoundsIntersect(strokeBounds, textBounds)) {
        continue;
      }

      const overlapWidth = Math.max(
        0,
        Math.min(strokeBounds[2], textBounds[2]) -
          Math.max(strokeBounds[0], textBounds[0]),
      );
      const overlapHeight = Math.max(
        0,
        Math.min(strokeBounds[3], textBounds[3]) -
          Math.max(strokeBounds[1], textBounds[1]),
      );
      const strokeOverlapRatio =
        (overlapWidth * overlapHeight) / strokeBoundsArea;

      if (strokeOverlapRatio < SCRIBBLE_DELETE_MIN_STROKE_BOUNDS_OVERLAP) {
        continue;
      }

      const insidePoints = points.reduce((count, point) => {
        if (
          point[0] >= textBounds[0] &&
          point[0] <= textBounds[2] &&
          point[1] >= textBounds[1] &&
          point[1] <= textBounds[3]
        ) {
          return count + 1;
        }
      ...
      const insideRatio = insidePoints / points.length;

      if (
        insidePoints < SCRIBBLE_DELETE_MIN_INSIDE_POINTS ||
        insideRatio < SCRIBBLE_DELETE_MIN_INSIDE_RATIO
      ) {
        continue;
      }

      const score = insideRatio + strokeOverlapRatio;
      if (score > bestScore) {
        bestScore = score;
        bestTarget = textElement;
      }
    }
  ...
  private eraseElements = (captureUpdate = true) => {
    let didChange = false;
